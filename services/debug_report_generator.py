        ...
        try:
            ...
            if employee_id:
                ...
                employees_response = self.sesame_api.get_employees(
                    page=1, per_page=300)
                all_employees = employees_response.get(
                    'data', []) if employees_response else []
            ...
            for i, employee in enumerate(employees, 1):
                ...
                page = 1
                while True:
                    self.log_api_call(f"get_work_entries (page {page})",
                                      emp_name)
                    ...
                    self.logger.info(
                        f"  ✓ Página {current_page}/{total_pages}: {len(page_entries)} entries (Total acumulado: {len(time_entries)}/{total_records})"
                    )
                ...
                if time_entries:
                    self.logger.info(
                        f"  ✓ TOTAL: {len(time_entries)} time entries obtenidos"
                    )
                else:
                ...
                while True:
                    ...
                    self.logger.info(
                        f"  ✓ Página {current_page}/{total_pages}: {len(page_breaks)} breaks (Total acumulado: {len(break_entries)}/{total_records})"
                    )
                ...
                if break_entries:
                    self.logger.info(
                        f"  ✓ TOTAL: {len(break_entries)} break entries obtenidos"
                    )
                else:
            ...
            ws.cell(row=current_row,
                    column=5,
                    value=f"Total API calls: {self.api_calls}")
            ws.cell(row=current_row,
                    column=6,
                    value=f"Total time: {time.time() - self.start_time:.1f}s")
