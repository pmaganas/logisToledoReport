...
class SesameAPI:
    ...
    def get_all_employees_data(self, company_id: str = None) -> List[Dict]:
        ...
        while True:
            try:
                response = self.get_employees(company_id=company_id,
                                              page=page,
                                              per_page=100)
                ...
                self.logger.info(
                    f"Fetched page {page}, total employees: {len(all_employees)}"
                )
                ...
                if page > 100:  # Max 10000 employees
                    self.logger.warning(
                        f"Hit safety limit of 100 pages. Total employees loaded: {len(all_employees)}"
                    )
                    break

            except Exception as e:
                self.logger.error(
                    f"Error fetching employees page {page}: {str(e)}")
                break

        self.logger.info(
            f"TOTAL EMPLOYEES LOADED: {len(all_employees)} employees across {page-1} pages"
        )
    ...
    def get_all_time_tracking_data(self,
        ...
        self.logger.info(
            f"WORK-ENTRIES: Starting fetch for employee {employee_id} from {from_date} to {to_date}"
        )

        while True:
            try:
                response = self.get_time_tracking(employee_id=employee_id,
                ...
                if not response:
                    self.logger.info(
                        f"WORK-ENTRIES: No response on page {page}, stopping pagination"
                    )
                    break

                if not response.get("data"):
                    self.logger.info(
                        f"WORK-ENTRIES: No data on page {page}, stopping pagination"
                    )
                    break
                ...
                if isinstance(entries, list):
                    ...
                    self.logger.info(
                        f"WORK-ENTRIES: Page {page} - got {len(entries)} entries, total so far: {len(all_entries)}"
                    )
                else:
                    all_entries.append(entries)
                    self.logger.info(
                        f"WORK-ENTRIES: Page {page} - got 1 entry, total so far: {len(all_entries)}"
                    )
                ...
                if len(entries) == 0:
                    self.logger.info(
                        f"WORK-ENTRIES: No entries in page {page}, stopping pagination"
                    )
                    break

                if current_page >= last_page:
                    self.logger.info(
                        f"WORK-ENTRIES: Reached last page ({last_page}), stopping pagination"
                    )
                    break
                ...
                self.logger.error(
                    f"Error fetching time tracking data page {page}: {str(e)}")
                break

        self.logger.info(
            f"WORK-ENTRIES: COMPLETED - Fetched {len(all_entries)} total entries across {page-1} pages"
        )
        return all_entries

    def get_all_breaks_data(self,
                            employee_id: str = None,
                            company_id: str = None,
                            from_date: str = None,
                            to_date: str = None) -> List[Dict]:
        ...
        self.logger.info(
            f"Starting break data fetch for employee {employee_id} from {from_date} to {to_date}"
        )

        while True:
            try:
                response = self.get_breaks(employee_id=employee_id,
                                           company_id=company_id,
                                           from_date=from_date,
                                           to_date=to_date,
                                           page=page,
                ...
                if not response or not response.get("data"):
                    self.logger.info(
                        f"No more break data on page {page}, stopping pagination"
                    )
                    break
                ...
                if isinstance(breaks, list):
                    ...
                    self.logger.info(
                        f"Fetched break page {page}, got {len(breaks)} breaks, total so far: {len(all_breaks)}"
                    )
                else:
                    all_breaks.append(breaks)
                    self.logger.info(
                        f"Fetched break page {page}, got 1 break, total so far: {len(all_breaks)}"
                    )
                ...
                self.logger.info(
                    f"Break page {current_page} of {last_page}, total items: {total_items}"
                )

                if page >= last_page:
                    self.logger.info(
                        f"Reached last break page ({last_page}), stopping pagination"
                    )
                    break
                ...
                self.logger.error(
                    f"Error fetching break data page {page}: {str(e)}")
                break

        self.logger.info(
            f"Completed break data fetch: {len(all_breaks)} total breaks")
        return all_breaks
    ...
    def get_all_offices_data(self, company_id: str = None) -> List[Dict]:
        ...
        while True:
            try:
                response = self.get_offices(company_id=company_id,
                                            page=page,
                                            per_page=per_page)
                ...
                self.logger.error(
                    f"Error getting offices data page {page}: {str(e)}")
                break
    ...
    def get_all_departments_data(self, company_id: str = None) -> List[Dict]:
        ...
        while True:
            try:
                response = self.get_departments(company_id=company_id,
                                                page=page,
                                                per_page=per_page)
                ...
                self.logger.error(
                    f"Error getting departments data page {page}: {str(e)}")
                break
    ...
    def get_all_check_types_data(self, company_id: str = None) -> List[Dict]:
        ...
        while True:
            try:
                result = self.get_check_types(company_id=company_id,
                                              page=page,
                                              per_page=100)
                ...
                self.logger.error(
                    f"Error getting check types data page {page}: {str(e)}")
                break
